...
public class Bird : MonoBehaviour
{
    ...
    public GameManager GameManager; // ссылка на менеджер для вызова методов обработки конца игры

    private Camera _camera; // ссылка на камеру 
    private Rigidbody2D _rigidbody; // ссылка на компонент физики
    private Quaternion _targetRotation; // целевой угол поворота птицы в кватернионах (ед измерения)
    private float _angleRotation = 15f; // угол поворота 
    private float _speedRotation = 5f;// скорость поворта 
    private float _upEdgeCamera; // верхняя граница камеры 
    ...
    private void Start()
    {
        ...
        float upOffsetCamera = 0.5f; // добавляем небольшой отступ от верхней границы камеры 
    ...
    private void BirdMovement()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0)) //если разово нажата ЛКМ
        {
            _rigidbody.velocity = Vector2.up * JumpForce; // придаём ускорение птицы через свойство velocity - скорость физического объекта
        ...
        if (transform.position.y > _upEdgeCamera) // если позиция птицы выше чем верхняя граница камеры
        {
            transform.position = new Vector2(transform.position.x, _upEdgeCamera); // не даё1м ей взлететь выше чем граница!
        }
       
    }

    private void OnTriggerEnter2D(Collider2D collision) // метод, который обрабатывает вход любого коллайдера в неосязаемую зону (триггер)
    {
        if (collision.CompareTag("Score")) // если птица проходит зону очков
        {
            GameManager.StartCoroutine(GameManager.ChangeText()); // меняем текст (размер и цвет)
        ...
        if (collision.CompareTag("Finish")) // если птица проходит зону финиша
        {
            ...
            GameManager.StartCoroutine(GameManager.CountDown()); // запускаем обратный отсчёт до перезагрузки сцены
        }
    }
}
